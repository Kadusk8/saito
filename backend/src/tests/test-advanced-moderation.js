...
async function runTests() {
    ...
    await db_1.supabase.from('members').delete().eq('jid', SENDER_JID).eq('group_id', '61aac9b3-a441-4bd0-98ab-4cc045468f1d');
    // Cleanup Redis keys for this group/sender to start fresh
    console.log('🧹 Cleaning up Redis keys...');
    const keys = await db_1.redisConnection.keys(`*:${GROUP_JID}:${SENDER_JID}*`);
    const groupKeys = await db_1.redisConnection.keys(`group_rules:${INSTANCE_NAME}:${GROUP_JID}*`);
    if (keys.length > 0)
        await db_1.redisConnection.del(...keys);
    if (groupKeys.length > 0)
        await db_1.redisConnection.del(...groupKeys);
    ...
    try {
        ...
        console.error('AI Blacklist test error:', e);
    }
    await sleep(5000);
    ...
    try {
        ...
        console.error('AI Topic test error:', e);
    }
...
//# sourceMappingURL=test-advanced-moderation.js.map