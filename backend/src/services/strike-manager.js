...
exports.StrikeManager = void 0;
const db_1 = require("../db");
class StrikeManager {
    static async addStrike(groupId, memberJid, reason, instanceName, groupJid) {
        console.log(`[STRIKE-MANAGER] Adding strike to member ${memberJid} in group ${groupId}. Reason: ${reason}`);
        // 1. Fetch member from Supabase
        const { data: member, error } = await db_1.supabase
            .from('members')
        ...
        if (!memberId) {
            ...
            const { data: newMember } = await db_1.supabase
                .from('members')
        ...
        await db_1.supabase.from('audit_logs').insert({
            group_id: groupId,
        ...
        if (currentStrikes === 1) {
            await db_1.evolution.sendText(instanceName, groupJid, `⚠️ Atenção @${memberJid.split('@')[0]}, você recebeu seu primeiro Aviso (1/3). Motivo: ${reason}. Evite novas infrações para não ser banido.`);
        }
        else if (currentStrikes === 2) {
            await db_1.evolution.sendText(instanceName, groupJid, `⛔️ PERIGO @${memberJid.split('@')[0]}, este é seu SEGUNDO AVISO (2/3). O próximo resultará em banimento imediato. Motivo: ${reason}.`);
        }
        else if (currentStrikes >= 3) {
            await db_1.evolution.sendText(instanceName, groupJid, `🚨 LIMITE EXCEDIDO. @${memberJid.split('@')[0]} foi banido do grupo após 3 violações. Motivo Final: ${reason}`);
            await db_1.evolution.removeParticipant(instanceName, groupJid, memberJid);
            // Log banimento final
            await db_1.supabase.from('audit_logs').insert({
                group_id: groupId,
                member_jid: memberJid,
    ...
    static async resetStrikes(memberId) {
        await db_1.supabase.from('members').update({ strikes: 0 }).eq('id', memberId);
    }
    static async incrementMessageCount(groupId, memberJid) {
        ...
        if (!member) {
            ...
            await db_1.supabase.from('members').insert({
                group_id: groupId,
                jid: memberJid,
                message_count: 1,
                lead_score: 1
            ...
            await db_1.supabase.from('members').update({
...
exports.StrikeManager = StrikeManager;
//# sourceMappingURL=strike-manager.js.map