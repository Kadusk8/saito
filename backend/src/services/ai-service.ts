...
export class AIService {
     ...
     */
    static async evaluateMessageIntent(message: string, groupBlacklist: string[]): Promise<{ isInfraction: boolean, reason: string }> {
    ...
    `;
        ...
        try {
            ...
            if (!content) return { isInfraction: false, reason: '' };

            return JSON.parse(content) as { isInfraction: boolean, reason: string };
        } catch (err) {
     ...
     */
    static async evaluateTopicConsistency(message: string, topic: string): Promise<{ isInfraction: boolean, reason: string }> {
     ...
     */
    static async chat(messages: any[], systemPrompt: string, temperature: number = 0.7): Promise<{ text: string }> {
     ...
     */
    static async refine(prompt: string): Promise<string> {
        if (!process.env.OPENAI_API_KEY) throw new Error('OPENAI_API_KEY not configured');
     ...
     */
    static async analyzeLead(message: string): Promise<{ classification: string, keywords_matched: string[], reasoning: string }> {
        if (!process.env.OPENAI_API_KEY) {
...
- cold: Apenas curioso ou sem intenção clara de compra`;
        ...
        try {
            ...
            const content = response.choices[0]?.message?.content;
            if (!content) return { classification: 'cold', keywords_matched: [], reasoning: '' };
