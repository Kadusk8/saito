...
async function aiRoutes(server) {
    ...
    server.get('/api/ai/chats', { preHandler: [auth_1.authenticate] }, async (request, reply) => {
    ...
    server.get('/api/ai/chats/:chatId/messages', { preHandler: [auth_1.authenticate] }, async (request, reply) => {
    ...
    server.post('/api/ai/chat', { preHandler: [auth_1.authenticate] }, async (request, reply) => {
        const { messages, chatId: existingChatId, title } = request.body;
        ...
        try {
            ...
            const aiResult = await ai_service_1.AIService.chat(messages, systemPrompt);
