...
- Nunca mencione que você é um modelo da OpenAI a menos que perguntado diretamente. Você é o Agente Saito.`;
            ...
            server.log.error({ err: error }, 'AI Chat Error');
            return reply.code(500).send({ error: error.message || 'Failed to generate AI response' });
        }
    });
}
